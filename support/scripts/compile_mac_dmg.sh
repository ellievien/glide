...
codesign --deep --force --verbose --options runtime --preserve-metadata=entitlements --sign "$SIGN_ID" build/macos/Build/Products/Release/Glide.app
...
create-dmg \
  ...
  --icon Glide.app 130 110 \
  --app-drop-link 360 110 \
  "$DMG" \
  build/macos/Build/Products/Release/Glide.app
